...
const FoodMenuSelection = ({ selectedMenus, setSelectedMenus, noOfPlates, extraPlates }) => {
    ...
    const toggleSelection = (menuKey) => {
        setSelectedMenus(prev => {
            ...
            // If not selected, add this menu while keeping others intact
            const categoryData = categories[menuKey] || {};
            const rate = String(categoryData.price ?? "");


            const base = parseInt(noOfPlates || "0", 10);
            const extra = parseInt(extraPlates || "0", 10);
            const total = Number(rate || 0) * (base + extra);


            // Collect all menuItems from subcategories
            ...
            return {
                ...prev, // ✅ preserve previously selected menus
                [menuKey]: {
                    rate,
                    noOfPlates: base,
                    extraPlates: extra,
    ...
    const handleRateChange = (menuKey, value) => {
        const base = Number(noOfPlates || 0);
        const extra = Number(extraPlates || 0);

        const total =
            value === "" || value === "."
                ? 0
                : Number(value) * (base + extra);

        setSelectedMenus(prev => ({
            ...prev,
            [menuKey]: {
                ...prev[menuKey],
                rate: value,        // 🔥 keep string
                noOfPlates: base,
                extraPlates: extra,
                total
            }
    ...
    return (
        <div className="food-menu-selection">
            ...
            {Object.keys(categories)
                .sort((a, b) => (selectedMenus[b] ? 1 : 0) - (selectedMenus[a] ? 1 : 0))
                .map((categoryName) => {
                            ...
                            {isSelected && (
                                <div style={{ marginTop: "12px", display: "flex", flexDirection: "column", gap: "10px" }}>
                                    ...
                                    >
                                        ...
                                        >
                                            ₹{formatIndianNumber((menuData?.total || 0).toFixed(2))}
                                        </span>
                                    </div>
                                </div>
                            )}
