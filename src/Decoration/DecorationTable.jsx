      ...
      (decorationSnap) => {
        ...
        const unsubscribePre = onSnapshot(
          ...
          (preSnap) => {
            ...
            // ✅ Filter prebookings: not in decoration + upcoming only
            const filteredPre = prebookings
              .filter((pre) => {
                ...
                const eventIST = new Date(eventDate.getTime() + istOffset);
                eventIST.setHours(0, 0, 0, 0);

                // ✅ NEW: amount filter
                const total = Number(pre.grandTotal || 0);

                // ✅ Show only future or today events (IST)
                return !decorationKeys.has(key) && eventIST >= istNow && total >= minDecorationAmount;
              })
              .map((pre) => {
                ...
                return {
                  ...
                  finalDate: formattedISTDate, // ✅ used for display/sort
                  startTime: "16:00",
                  endTime: "21:00",
                  source: "Shangrila",
                };
