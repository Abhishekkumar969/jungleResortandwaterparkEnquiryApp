            ...
            (vendorSnap) => {
                ...
                const makeKey = (name, contact, eventType, date) => {
                    ...
                    return `${(name || "").trim().toLowerCase()}|${(contact || "")
                        .replace(/\s+/g, "")
                        .replace(/[^\d]/g, "")}|${(eventType || "").trim().toLowerCase()}|${dateKey}`;
                };
                    ...
                    (preSnap) => {
                        ...
                        const filteredPre = prebookings
                            .filter((pre) => {
                                ...
                                return !vendorKeys.has(key) && total >= minEventAmount;
                            })
                            .map((pre) => {
                        ...
                        // Sort latest first
                        filteredPre.sort((a, b) => toIST(b.finalDate || 0) - toIST(a.finalDate || 0));
                            ...
                            <table className="leads-table" style={{ height: "fit-content", maxHeight: "70vh" }}>
                                <thead>
                                    <tr>
                                        ...
                                        >
                                            Event Date{" "}
                                            {sortField === "finalDate" ? (sortOrder === "asc" ? "▲" : "▼") : ""}
                                        </th>
                                        <th>Name</th>
                                        <th
                                            style={{ cursor: "pointer" }}
                                        ...
                                        >
                                            Booked On{" "}
                                            {sortField === "enquiryDate" ? (sortOrder === "asc" ? "▲" : "▼") : ""}
                                        </th>
                                        <th>Contact</th>
                                        <th>Event Type</th>
                                        <th>Venue Type</th>
                                        <th>Time</th>
                                        {userAppType === 'C' && (<th>Book / Drop</th>)}
                                    </tr>
                                </thead>
