...
function BookingRow({ v, idx, filteredCount, getAdvanceTotal, getGrandTotal, normalizeServices, navigate, getPrintFormat, handleAddAmountClick }) {
  ...
  const handleUpdateAdvance = async (index, newAmount) => {
    try {
      const updatedAdvances = [...(v.advance || [])];

      const oldAmount = updatedAdvances[index].amount;
      updatedAdvances[index].amount = Number(newAmount);

      const ref = doc(db, "vendor", v.monthYear);

      // 🔥 find next log number
      const existingLogKeys = Object.keys(v).filter(k => k.startsWith("updateLog"));
      const nextLogNumber = existingLogKeys.length + 1;
      const newLogKey = `updateLog${nextLogNumber}`;

      // 🔥 log structure (same as your system)
      const logEntry = {
      ...
      await updateDoc(ref, {
        [`${v.id}.advance`]: updatedAdvances,

        // 🔥 IMPORTANT FIX
        [`${v.id}.${newLogKey}`]: logEntry,
      });
  ...
  return (
    <>
      <tr key={v.id}
        style={{ backgroundColor: idx % 2 === 0 ? "#ffffff" : "#eaf4ff", }}>
        <td style={{ backgroundColor: idx % 2 === 0 ? "#ffffff" : "#eaf4ff", }}>{filteredCount - idx}</td>
        <td style={{ backgroundColor: idx % 2 === 0 ? "#ffffff" : "#eaf4ff", }}>{v.finalDate ? (() => {
        ...
        <td style={{ backgroundColor: idx % 2 === 0 ? "#ffffff" : "#eaf4ff", }}>{v.bookedOn ? (() => {
          const [y, m, d] = v.bookedOn.split("-").map(Number);
          return `${String(d).padStart(2, "0")}-${String(m).padStart(2, "0")}-${y}`;
        })() : "-"}</td>

        <td style={{ backgroundColor: idx % 2 === 0 ? "#ffffff" : "#eaf4ff", }}>{v.address}</td>
                  ...
                  {editingAdvance?.index !== i ? (
                    ...
                    <>
                      ...
                      <div style={{ display: "flex", gap: "5px", marginTop: "5px" }}>
                        <button
                          onClick={() =>
                            handleUpdateAdvance(i, editingAdvance.value)
                            &&
                            setEditingAdvance(null)
                          }
